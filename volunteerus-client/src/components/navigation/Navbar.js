        ...
        </div>
        {
          navigation.map((item) => (
            <Link 
              key={item.name} 
              to={item.href} 
              className={ classNames(item.current ? 'bg-pink-300 text-black' : 'text-gray-500', 'flex items-center px-4 py-2 text-sm sm:text-base font-medium hover:text-gray-900 hover:bg-neutral-200')}
            >
              <item.icon className="h-6 w-6 mr-3" aria-hidden="true" />
              {item.name}
            </Link>
