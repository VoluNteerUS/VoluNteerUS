...
function EditEventPart4({ formQuestions, setFormQuestions, error, setError, setPage, handleSubmit }) {
            ...
            <div className="bg-white rounded-lg"> 
              ...
              <div className="grid grid-cols-12 gap-4"> 
                <div className="col-span-12 px-6 sm:px-12 xl:py-2 mx-0 sm:mx-4 flex flex-col"> 
                  <h1 className="font-bold font-serif tracking-tight leading-none text-darkblue-900 text-xl md:text-2xl xl:text-3xl mb-6">Edit sign up form for event</h1> 
                  {/* form progress bar */}
                  ...
                  <form onSubmit={ handleSubmit } className="flex flex-col space-y-5">
                    {Object.values(formQuestions).map((question) => ( 
                      <>
                        <div key={ question[0] } className="grid grid-cols-12 gap-3">
                          <input type="text" value={ question[1] } onChange={ e => handleQuestionChange(e, question) } placeholder={`Question ${ question[0] }`} className="border border-grey-600 p-1 rounded-lg col-span-12 md:col-span-7"/>
                          <select value={ question[2] } onChange={ e => handleTypeChange(e, question) } className="border border-grey-600 p-1 rounded-lg col-span-8 md:col-span-3">
                            <option value="Open-ended">Open-ended</option>
                            <option value="MCQ">MCQ</option>
                            <option value="MRQ">MRQ</option>
                          </select>
                          <button type="button" onClick={ e => handleRemoveQuestion(e, question) } className="bg-red-700 text-white rounded-lg px-2 col-span-4 md:col-span-2">Remove</button>
                        </div>
                        <div>
