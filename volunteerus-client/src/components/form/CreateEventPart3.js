...
function CreateEventPart3({ details, setDetails, error, setError, setPage, handleSubmit }) {
  const handleBack = () => {
    setError("");
    setPage(2);
  }
  ...
  const handleChange = (e, question) => {
    ...
    setError("");
    if ((question.index === 0 && e.target.value === "No") || (details.group[0] == "No" && e.target.value != "Yes")) {
      if (question.index === 1) {
      ...
      setDetails({ ...details, group: newGroupDetails })
    } else {
      let newGroupDetails = details.group;
            ...
            <div className="col-span-12 sm:px-12 xl:py-2 mx-0 sm:mx-4 flex flex-col"> 
              ...
              <form onSubmit={ handleSubmit } className="mt-16 space-y-5">
                {groupingQuestions.map((question) => (
                  <>
                    <div className="grid sm:grid-cols-4"> 
                      ...
                      <select value={details.group[question.index]} onChange={(e) => handleChange(e, question) } className="border border-grey-600 p-1 rounded-lg">
                ...
                <div className="grid sm:grid-cols-4 py-10">
                  <button type="submit" className="bg-pink-400 text-white rounded-lg px-3 py-1 col-start-4">Submit</button>
                </div>
              </form>
            </div>
          </div>
